...
def compute_edits_and_insertions(
    ...
    Args:
      ...
      max_insertions_per_token: Maximum number of tokens to insert per source
        token.
      insert_after_token: Whether to insert after the current token (the current behavior on text editing model) or before it (the current behavior in LaserTagger and in the original experimental Insert implementation).
    ...
    if insert_after_token:
        return _get_edits_and_insertions(
            kept_tokens, source_tokens, added_phrases, max_insertions_per_token
        )
    else:
        ...
        edits_and_insertions = _get_edits_and_insertions(
            kept_tokens[::-1],
            source_tokens[::-1],
            _reverse_list_of_lists(added_phrases),
            max_insertions_per_token,
        )
...
def _get_added_token_lists(kept_tokens, target_tokens):
    ...
    for token in source_tokens:
        if kept_idx < len(kept_tokens) and token == kept_tokens[kept_idx]:
            ...
            if (
                ...
                and edit_operations[-2] == constants.KEEP
                and insertions[-1]
            ):
