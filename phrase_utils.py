...
def _get_added_phrases(source_tokens, target_tokens):
...
def added_token_counts(
    ...
    num_examples = 0
    all_added_phrases = []
    for i, (sources, target) in enumerate(data_iterator):
        if num_examples >= max_input_examples:
            break
        if i % 1000 == 0:
            print(f"{num_examples} examples processed.")
        ...
        with _recursion_limit(max_recursion_depth):
            try:
                ...
                if try_swapping and len(sources) == 2:
                    ...
                    added_phrases_swap = _get_added_phrases(
                        source_tokens_swap, target_tokens
                    )
...
def construct_added_phrases_matrix(all_added_phrases, phrase_counter):
    ...
    matrix = scipy.sparse.dok_matrix(
        (len(all_added_phrases), len(phrase_2_idx)), dtype=bool
    )
